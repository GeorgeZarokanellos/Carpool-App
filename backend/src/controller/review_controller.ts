...
export const createReview = async (req: Request, res: Response, next: NextFunction): Promise<void> => {
        await sequelize.transaction(async (transaction: Transaction) => {
            ...
            console.log("reviewed user id: " + reviewedUserId + " " + "trip id: " + tripId);
            ...
            if(UserToUpdate != null){
                console.log("no of reviews before update: " + UserToUpdate.noOfReviews);
                UserToUpdate.noOfReviews += 1;  //increment the number of reviews of the user
                console.log("no of reviews after update: " + UserToUpdate.noOfReviews);
...
const calculateAverageRatingForNewReview = async (reviewRating: number, reviewedUserId: number): Promise<void> => {
    try {       
        ...
        if(userToUpdate != null){
            console.log("User found: " + userToUpdate.userId);
            let newOverallRating = 0;
            const userReviews = userToUpdate.noOfReviews;
            console.log("Total reviews: " + userReviews);
            console.log("Overall rating before update: " + userToUpdate.overallRating);
            ...
            if(userReviews != 0)
                {
                    ...
                    console.log("increase or decrease in overall rating: " + (reviewRating/userReviews));
            ...
            console.log("New overall rating: " + newOverallRating);
        ...
        if(typeof error === 'string'){
            console.error(error);
            console.log("There was an error updating the average rating of the user: " + error);
...
const calculateAverageRatingForUpdatedReview = async (reviewId: number, reviewRating: number, reviewedUserId: number): Promise<void> => {
    try {
        ...
        if(review != null && userToUpdate != null){
            ...
            console.log("Old review rating: " + oldReviewRating);
            console.log("Total reviews: " + userReviews);
            console.log("Overall rating before update: " + overallRating);
            //calculate the new overall rating
            if(userReviews != 0){
            ...
            console.log("New overall rating: " + overallRating);
            //update the user's overall rating
            userToUpdate.overallRating = newOverallRating;
            userToUpdate.save();
        } else {
